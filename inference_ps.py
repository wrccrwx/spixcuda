...
@torch.no_grad()
def ssn_inference(image, nspix, n_iter, fdim=None, color_scale=0.26, pos_scale=2.5, weight=None, enforce_connectivity=True):
    """
    ...
    """
    image = np.squeeze(image, 0).transpose(1, 2, 0)
    model = lambda data: sparse_ssn_iter(data, nspix, n_iter)
...
@torch.no_grad()
def color_inference(image, nspix, n_iter, fdim=None, color_scale=0.26, pos_scale=2.5, weight=None, enforce_connectivity=True):
    """
    generate superpixels
...
if __name__ == "__main__":
    ...
    s = time.time()
    label = ssn_inference(image, args.nspix, args.niter, args.fdim, args.color_scale, args.pos_scale, args.weight)
